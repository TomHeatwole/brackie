...
export default async function CreatePoolPage({
  ...
  if (!testMode && isActiveOrCompleted) {
    redirect(`/pools${modeSuffix}`);
  }

  return (
    <div className="min-h-screen bg-background">
      <Navbar userEmail={user.email} firstName={userInfo?.first_name} lastName={userInfo?.last_name} avatarUrl={userInfo?.avatar_url} activeTab="Pools" modeParam={modeSuffix} />
      <main className="pt-20 min-h-screen flex flex-col items-center pb-24">
        <div className="w-full max-w-md px-4 py-8">
          <Link
            href={`/pools${modeSuffix}`}
